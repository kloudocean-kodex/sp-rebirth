    ...
    defineField({
      ...
      validation: (rule) =>
        rule
          .max(70)
          .warning(
            "Aim for a concise search title; Google may rewrite long titles.",
          ),
    }),
    defineField({
      name: "description",
      ...
      validation: (rule) =>
        rule
          .max(180)
          .warning(
            "Keep this concise enough to display well in search results.",
          ),
    }),
    defineField({
      name: "canonical",
      ...
      type: "url",
      description:
        "Usually leave blank. Use only when the canonical must differ from the page URL.",
    }),
    defineField({
      name: "noindex",
...
export const accessibleImage = defineType({
  ...
  fields: [
    ...
    defineField({
      ...
      initialValue: false,
      description:
        "Enable only when the image conveys no information and should have an empty alt attribute.",
    }),
