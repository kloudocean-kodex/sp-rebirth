...
export async function startBrowserTestServer({ publicPort = 8788, workerPort = 8789 } = {}) {
  ...
  const proxy = createServer((incoming, outgoing) => {
    const requestHeaders = Object.fromEntries(
      Object.entries(incoming.headers).filter(([name, value]) => value !== undefined && !HOP_BY_HOP_HEADERS.has(name)),
    );
    ...
    const upstream = httpRequest(
      ...
      (upstreamResponse) => {
        ...
        if (upstreamResponse.statusMessage) outgoing.statusMessage = upstreamResponse.statusMessage;
    ...
    upstream.on('error', (error) => {
      ...
      outgoing.end(`Browser QA Worker is not ready: ${error.code ?? 'upstream-error'}`);
    });
  ...
  const stop = (signal = 'SIGTERM') => {
    ...
    stopping = true;
    proxy.close();
    if (wrangler.exitCode === null && wrangler.signalCode === null) wrangler.kill(signal);

    const forceStop = setTimeout(() => {
      if (wrangler.exitCode === null && wrangler.signalCode === null) wrangler.kill('SIGKILL');
    }, 5_000);
    forceStop.unref();
  };
  ...
  wrangler.once('exit', (code, signal) => {
    ...
    console.error(`Browser QA Wrangler exited unexpectedly (${signal ?? code ?? 'unknown'}).`);
